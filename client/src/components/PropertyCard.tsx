...
export default function PropertyCard({
  ...
  return (
    <Card
      className="group hover:shadow-xl transition-all duration-300 rounded-xl overflow-hidden border border-gray-200 hover:border-blue-300 bg-white h-full flex flex-col"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Image Carousel */}
      ...
      <CardContent className="p-4 flex-1 flex flex-col">
        ...
        {/* Price - Using IndianRupee icon */}
        <div className="flex items-center gap-1 mb-2">
          <IndianRupee className="h-4 w-4 text-emerald-600 flex-shrink-0" />
          <span className="text-lg font-bold text-gray-900">{formatPrice(price)}</span>
          {rent_type && (
            <span className="text-xs text-gray-500 ml-1">/{rent_type}</span>
          )}
